...
from database import db_instance, LOCAL_USER_ID  # Import local user ID
        ...
        while True:
            async with self._lock:
                ...
                if len(self._minute_timestamps) >= self.rpm_limit:
                    wait_rpm = (
                        self._minute_timestamps[0] + timedelta(minutes=1) - now
                    ).total_seconds()
                else:
                    wait_rpm = 0
                ...
                if current_minute_tokens + tokens_to_add > self.tpm_limit:
                    ...
                    if self._token_counts:
                        wait_tpm = (
                            self._token_counts[0][0] + timedelta(minutes=1) - now
                        ).total_seconds()
                    else:
                ...
                if len(self._day_timestamps) >= self.rpd_limit:
                    wait_rpd = (
                        self._day_timestamps[0] + timedelta(days=1) - now
                    ).total_seconds()
                else:
                    wait_rpd = 0
            ...
            if wait_time > 0:
                logging.getLogger(__name__).debug(
                    f"Rate limiting: waiting for {wait_time:.2f} seconds."
                )
...
class CodexExtraction(BaseModel):
    new_items: List[CodexItem] = Field(
        default_factory=list, description="List of new codex items found in the chapter"
    )
...
class ChapterGenerationState(TypedDict):
    # Inputs
    # user_id: str # Removed, will use LOCAL_USER_ID implicitly
    project_id: str
    chapter_number: int
    plot: str
    writing_style: str
...
class AgentManager:
    ...
    # Store the local user ID
    user_id = LOCAL_USER_ID

    def __init__(self, project_id: str, api_key_manager: ApiKeyManager):
        self.logger = logging.getLogger(__name__)
        self.logger.setLevel(logging.DEBUG)  # Set higher level in production
        # self.user_id = user_id # Removed, use class attribute
    ...
    @classmethod
    async def create(
        cls, project_id: str, api_key_manager: ApiKeyManager
    ) -> "AgentManager":
        """Factory method to create and initialize AgentManager asynchronously."""
        key = (cls.user_id, project_id)  # Use class user_id for key
        if key in agent_managers:
            return agent_managers[key]

        instance = cls(project_id, api_key_manager)  # Pass only project_id
    ...
    async def initialize(self):
        ...
        self.logger.info(
            f"Initializing AgentManager for Project: {self.project_id[:8]} (Local User: {self.user_id[:8]})"
        )
        try:
            ...
            # Initialize Vector Store (user_id is now class attribute)
            self.vector_store = VectorStore(
                self.user_id,  # Use class attribute
                self.project_id,
                self.api_key,
            ...
            self.logger.info(
                f"AgentManager Initialized for Project: {self.project_id[:8]} (Local User: {self.user_id[:8]})"
            )
    ...
    async def close(self):
        ...
        self.logger.info(
            f"Closing AgentManager for Project: {self.project_id[:8]} (Local User: {self.user_id[:8]})"
        )
        ...
        self.logger.info(
            f"AgentManager closed for Project: {self.project_id[:8]} (Local User: {self.user_id[:8]})"
        )
    ...
    async def _get_model_settings(self) -> dict:
        ...
        try:
            # settings = await db_instance.get_model_settings(self.user_id) # Removed user_id
            ...
            self.logger.error(f"Error getting local model settings: {e}", exc_info=True)
            raise
    ...
    async def _construct_context_node(
        ...
        try:
            ...
            try:
                ...
                previous_chapters_data = await db_instance.get_all_chapters(project_id)
                ...
                if recent_chapters:
                    ...
                    for chapter in recent_chapters:
                        ...
                        # Summarize if long
                        if (
                            self.estimate_token_count(content) > 1000
                        ):  # Summarize if > 1k tokens
                            docs_to_summarize = [Document(page_content=content)]
                            summary_result = await state["summarize_chain"].arun(
                                docs_to_summarize
                            )
                            context_parts.append(
                                f"- Ch {chap_num} ({chap_title}): {summary_result}"
                            )
                        else:
                            context_parts.append(
            ...
            # Fetch relevant codex items from Vector Store (user_id handled by vector_store instance)
            try:
    ...
    def _create_chapter_prompt(
        self, instructions: Dict[str, Any], context: str
    ) -> ChatPromptTemplate:
        ...
        *   If specific sentence structures or endings are required, apply them CONSISTENTLY.
        ...
        """
        human_template = (
            "Write the chapter following all system instructions precisely."
        )
    ...
    async def _generate_initial_chapter_node(
        ...
        try:
            ...
            target_word_count = instructions.get(
                "wordCount", 0
            )  # Get target WC from instructions
            ...
            chapter_content = await chain.ainvoke(
                {
                    ...
                    "additional_instructions": instructions.get(
            ...
            extension_text = await chain.ainvoke(
                {
                    ...
                    "context": state[
                        "context"
                    ],  # Provide context again for consistency
                }
            )
    ...
    async def _extract_codex_items_node(
        ...
        try:
            ...
            self.logger.debug(
                f"Found {len(existing_names)} existing codex item names for filtering."
            )
            ...
            chain = (
                prompt | check_llm | JsonOutputParser(pydantic_object=CodexExtraction)
            ...
            try:
                ...
                self.logger.warning(
                    f"JSON/Pydantic parsing failed for codex extraction: {parse_error}. Attempting fix."
                )
                ...
                self.logger.error(
                    f"Codex extraction LLM call failed: {invoke_error}", exc_info=True
                )
            ...
            for item in all_new_items_raw:
                ...
                if item.subtype:
                    try:
                        ...
                        self.logger.warning(
                            f"Invalid worldbuilding subtype '{item.subtype}' for item '{item.name}'. Skipping subtype."
                        )
                ...
                if item_name_lower not in unique_items_dict:
                    item_dict = {
                        ...
                        "subtype": (
                            item_subtype_enum.value if item_subtype_enum else None
                        ),
                    }
            ...
            self.logger.info(
                f"Extracted {len(final_new_items)} unique new codex items."
            )
    ...
    async def add_to_knowledge_base(
        ...
        try:
            ...
            if ids:
                embedding_id = ids[0]
                self.logger.debug(
                    f"Added '{content_type}' item to KB. Embedding ID: {embedding_id}"
                )
                return embedding_id
            else:
                self.logger.error(
        ...
        try:
            ...
            if isinstance(identifier, str):
                ...
                embedding_id = await self.vector_store.get_embedding_id(
                    item_id=identifier["item_id"], item_type=identifier["item_type"]
                )
                if not embedding_id:
                    self.logger.warning(
    ...
    async def query_knowledge_base(
        ...
        try:
                ...
                if history_messages:
                    condense_prompt = ChatPromptTemplate.from_messages(
                        [
                            ...
                            (
                                "human",
                                "Given the chat history and the follow up question, rephrase the follow up question to be a standalone question.\n\nFollow Up Question: {question}\nStandalone question:",
                            ),
                        ]
                    )
                    ...
                    self.logger.debug(
                        f"Original Query: {query}, Standalone Query: {standalone_question}"
                    )

            # 2. Retrieve Documents
            retrieved_docs = await self.vector_store.similarity_search(
                standalone_question, k=5
            )  # Reduced k for smaller context
            ...
            context_str = "\n\n---\n\n".join(
                [
                    f"Source {i+1} ({doc.metadata.get('type', 'doc')} - {doc.metadata.get('name', 'unnamed')}):\n{doc.page_content}"
            ...
            answer_prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        "You are an assistant answering questions based SOLELY on the provided context. If the answer isn't in the context, say you don't know. Be concise.",
                    ),
                    ("human", "Context:\n{context}\n\nQuestion: {question}\n\nAnswer:"),
                ]
            )
            ...
            answer = await answer_chain.ainvoke(
                {"context": context_str, "question": standalone_question}
            )
    ...
    async def generate_codex_item(
        ...
        try:
            ...
            existing_items = await db_instance.get_all_codex_items(self.project_id)
    ...
    async def analyze_character_relationships(
        ...
        try:
            ...
            all_chapters_data = await db_instance.get_all_chapters(self.project_id)
            context_content = "\n\n".join(
                [
            ...
            if context_tokens > max_context_tokens:
                ...
                context_content = await self.summarize_chain.arun(
                    docs_to_summarize
                )  # Use initialized summarize chain
    ...
    async def extract_character_backstory(
    ...
    ) -> Optional[CharacterBackstoryExtraction]:
        ...
        try:
            character_data = await db_instance.get_characters(
                self.project_id, character_id=character_id
            )
            if (
                not character_data
                or len(character_data) == 0
                or character_data[0]["type"] != CodexItemType.CHARACTER.value
            ):
                self.logger.error(
                    f"Character {character_id} not found or not a character."
                )
                return None
            character_data = character_data[0]  # Get the first character if found

            # Get all chapters not yet processed for backstory
            chapters_to_process = (
                await db_instance.get_latest_unprocessed_chapter_content(
                    self.project_id, PROCESS_TYPES["BACKSTORY"]  # Use constant
                )
            )

            if not chapters_to_process:
                self.logger.info(
                    f"No unprocessed chapters found for backstory extraction for character {character_id}."
                )
                return CharacterBackstoryExtraction(
                    character_id=character_id, new_backstory=""
                )

            self.logger.info(
                f"Processing {len(chapters_to_process)} chapters for backstory of {character_data['name']}."
            )
            ...
            fixing_parser = OutputFixingParser.from_llm(
                parser=parser, llm=backstory_llm
            )
            ...
            for chapter in chapters_to_process:
                ...
                try:
                    ...
                    result = await chain.ainvoke(
                        {
                            ...
                            "existing_backstory": character_data.get(
                                "backstory", "None provided yet."
                            ),
                    ...
                    if result and result.new_backstory and result.new_backstory.strip():
                        ...
                        self.logger.debug(
                            f"Found new backstory in chapter {chapter_id} for {character_data['name']}"
                        )
                    # else: Chapter had no new info, implicitly processed

                except Exception as chapter_error:
                    self.logger.error(
                        f"Error processing chapter {chapter_id} for backstory: {chapter_error}",
                        exc_info=True,
                    )
            ...
            if combined_backstory:
                await db_instance.save_character_backstory(
                    character_id, combined_backstory, self.project_id
                )
                ...
                self.logger.info(
                    f"Updated backstory for character {character_id} with new info."
                )

            return CharacterBackstoryExtraction(
                character_id=character_id, new_backstory=combined_backstory
            )

        except Exception as e:
            self.logger.error(
                f"Error extracting character backstory for {character_id}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def analyze_unprocessed_chapter_locations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            chapters_to_process = (
                await db_instance.get_latest_unprocessed_chapter_content(
                    self.project_id, PROCESS_TYPES["LOCATIONS"]
                )
            )
            ...
            existing_locations_data = await db_instance.get_locations(self.project_id)
            existing_location_names = {
                loc["name"].lower() for loc in existing_locations_data
            }
            self.logger.debug(
            ...
            existing_names_str = (
                ", ".join(list(existing_location_names))
                ...
                try:
                    ...
                    if result and hasattr(result, "locations"):
                        for loc_analysis in result.locations:
                            ...
                            if loc_name_lower not in existing_location_names:
                                try:
                                    ...
                                    location_dict["id"] = (
                                        db_location_id  # Update dict with real ID
                                    )
                                    location_dict["description"] = (
                                        loc_analysis.significance_analysis
                                    )  # Align field name if needed
                                    ...
                                    self.logger.error(
                                        f"Error saving/adding new location '{loc_analysis.location_name}': {db_kb_error}"
                                    )
                        ...
                        if new_locations_in_chapter:
                            ...
                            self.logger.info(
                                f"Found {len(new_locations_in_chapter)} new locations in chapter {chapter_id}."
                            )

                        processed_chapter_ids.append(
                            chapter_id
                        )  # Mark chapter as processed for locations

                except Exception as chapter_error:
                    self.logger.error(
                        f"Error processing chapter {chapter_id} for locations: {chapter_error}",
                        exc_info=True,
                    )
            ...
            for ch_id in processed_chapter_ids:
                await db_instance.mark_chapter_processed(
                    ch_id, PROCESS_TYPES["LOCATIONS"]
                )

            self.logger.info(
                f"Location analysis complete. Found {len(all_new_locations)} new locations total."
            )
    ...
    async def analyze_unprocessed_chapter_events(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            chapters_to_process = (
                await db_instance.get_latest_unprocessed_chapter_content(
                    self.project_id, PROCESS_TYPES["EVENTS"]
                )
            )
            ...
            existing_events_data = await db_instance.get_events(self.project_id)
            existing_event_titles = {
                event["title"].lower() for event in existing_events_data
            }
            ...
            existing_titles_str = (
                ", ".join(list(existing_event_titles))
            ...
            for chapter in chapters_to_process:
                ...
                try:
                            ...
                            if event_title_lower not in existing_event_titles:
                                try:
                                    ...
                                    if event_desc.involved_characters:
                                        ...
                                        char_data_list = (
                                            await db_instance.get_characters(
                                                self.project_id,
                                                name=event_desc.involved_characters[0],
                                            )
                                        )
                                        if char_data_list:
                                            character_id = char_data_list[0]["id"]

                                    location_id = None
                                    if event_desc.location:
                                        loc_data = (
                                            await db_instance.get_location_by_name(
                                                event_desc.location, self.project_id
                                            )
                                        )
                                        if loc_data:
                                    ...
                                    db_event_id = await db_instance.create_event(
                                        ...
                                        date=datetime.now(
                                            timezone.utc
                                        ),  # Placeholder date, needs better handling
                                        project_id=self.project_id,
                                    ...
                                    await self.add_to_knowledge_base(
                                        "event", event_desc.description, kb_metadata
                                    )
                                    ...
                                    self.logger.error(
                                        f"Error saving/adding new event '{event_desc.title}': {db_kb_error}"
                                    )

                        if new_events_in_chapter:
                            all_new_events.extend(new_events_in_chapter)
                            self.logger.info(
                                f"Found {len(new_events_in_chapter)} new events in chapter {chapter_id}."
                            )
                    ...
                    self.logger.error(
                        f"Error processing chapter {chapter_id} for events: {chapter_error}",
                        exc_info=True,
                    )

            # Mark successfully processed chapters
            for ch_id in processed_chapter_ids:
                await db_instance.mark_chapter_processed(ch_id, PROCESS_TYPES["EVENTS"])

            self.logger.info(
                f"Event analysis complete. Found {len(all_new_events)} new events total."
            )
    ...
    async def analyze_event_connections(self) -> List[EventConnection]:
        ...
        try:
            events = await db_instance.get_events(self.project_id, limit=100)
            if len(events) < 2:
            ...
            fixing_parser = OutputFixingParser.from_llm(
                parser=parser, llm=connection_llm
            )
                ...
                [
                    {
                        "id": e["id"],
                        "title": e["title"],
                        "desc_snippet": e["description"][:100] + "...",
                    }
                    for e in events
                ],
                indent=2,
            )
            existing_connections_json = json.dumps(
                [
                    {
                        "e1": c["event1_id"],
                        "e2": c["event2_id"],
            ...
            if result and hasattr(result, "connections"):
                ...
                for conn in deduped_result.connections:
                    ...
                    if not conn.event1_id or not conn.event2_id:
                        continue  # Skip if LLM hallucinated IDs
                    ...
                    if conn_key not in existing_keys:
                        try:
                            ...
                            self.logger.error(
                                f"Error saving/adding event connection between {conn.event1_id} and {conn.event2_id}: {db_kb_error}"
                            )

            self.logger.info(
                f"Event connection analysis complete. Found {len(saved_connections_output)} new connections."
            )
    ...
    async def analyze_location_connections(self) -> List[LocationConnection]:
        ...
        try:
            locations = await db_instance.get_locations(self.project_id)
            if len(locations) < 2:
            ...
            self.logger.debug(
                f"Found {len(existing_keys)} existing location connections."
            )
                ...
                [
                    {
                        "id": loc["id"],
                        "name": loc["name"],
                        "desc_snippet": loc["description"][:100] + "...",
                    }
            ...
            if result and hasattr(result, "connections"):
                ...
                for conn in deduped_result.connections:
                    # Final check against existing DB keys
                    if not conn.location1_id or not conn.location2_id:
                        continue
                    ...
                    if conn_key not in existing_keys:
                        try:
                            ...
                            loc1 = next(
                                (l for l in locations if l["id"] == conn.location1_id),
                                None,
                            )
                            loc2 = next(
                                (l for l in locations if l["id"] == conn.location2_id),
                                None,
                            )
                            if not loc1 or not loc2:
                                self.logger.warning(
                                    f"Could not find location data for IDs {conn.location1_id} or {conn.location2_id}. Skipping connection."
                                )
                                continue
                            ...
                            if conn.travel_route:
                                kb_content += f" Travel: {conn.travel_route}."
                            if conn.cultural_exchange:
                                kb_content += f" Culture: {conn.cultural_exchange}."
                            ...
                            self.logger.error(
                                f"Error saving/adding location connection between {conn.location1_id} and {conn.location2_id}: {db_kb_error}"
                            )

            self.logger.info(
                f"Location connection analysis complete. Found {len(saved_connections_output)} new connections."
            )
            return saved_connections_output

        except Exception as e:
            self.logger.error(
                f"Error analyzing location connections: {e}", exc_info=True
            )
            raise
    ...
    async def get_chat_history(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            history = await db_instance.get_chat_history(self.project_id)
    ...
    async def reset_memory(self):
        ...
        await db_instance.delete_chat_history(self.project_id)
    ...
    async def reset_knowledge_base(self):
        ...
        self.logger.info(
            f"Knowledge base reset complete. Restored {len(restored_items)} items from backup."
        )
    ...
    for manager in managers_to_close:
        try:
            ...
            logging.getLogger(__name__).error(
                f"Error closing manager for user {manager.user_id}: {e}"
            )
