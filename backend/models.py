...
class CharacterBackstoryExtraction(BaseModel):
    character_id: str = Field(..., description="ID of the character whose backstory is being extracted")
    new_backstory: str = Field(..., description="New backstory content extracted from the chapter")

class RelationshipAnalysis(BaseModel):
    character1: str = Field(..., description="Name of the first character in the relationship")
    character2: str = Field(..., description="Name of the second character in the relationship")
    relationship_type: str = Field(..., description="Type of relationship between the characters")
    description: str = Field(..., description="Description of the relationship dynamics")
...
class LocationConnection(BaseModel):
    ...
    location1_name: str
    location2_name: str
    connection_type: str = Field(..., description="Type of connection between locations")
    description: str = Field(..., description="Description of how the locations are connected")
    travel_route: Optional[str] = Field(None, description="Description of travel route between locations")
    cultural_exchange: Optional[str] = Field(None, description="Description of cultural exchange between locations")
...
class LocationAnalysis(BaseModel):
    ...
    significance_analysis: str = Field(..., description="Analysis of the location's significance")
    connected_locations: List[str] = Field(default_factory=list, description="Names of connected locations")
    notable_events: List[str] = Field(default_factory=list, description="Notable events that occurred here")
    character_associations: List[str] = Field(default_factory=list, description="Characters associated with this location")
...
class EventConnection(BaseModel):
    ...
    description: str = Field(..., description="Description of how the events are connected")
    impact: str = Field(..., description="Impact or significance of this connection")
    characters_involved: Optional[str] = Field(None, description="Characters involved in connecting the events")
    location_relation: Optional[str] = Field(None, description="Spatial relationship between events")
